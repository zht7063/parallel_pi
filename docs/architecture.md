...
本节保留完整验收计划；Linux 已执行的探针、已知缺口与未验证项见 [验证记录](validation.md)。真实 provider 与 macOS 仍未执行。V01–V04 在 Linux 和 macOS 分别记录结果；任一平台缺少运行证据时，不能报告该平台已通过。单元替身用于状态机与故障注入；真实固定版本内核用于兼容结论，两者不能互相替代。
...
当前继续采用原生 RPC；硬终止后 Bash 可能残留，平台监督不能只杀 pi 进程组。模型启动需核对真实可用目录。配置/MWF 修订检查已验证窄适配路径，但涉及固定版本内部 API，必须保留升级契约检查。真实模型与 macOS 门槛尚未通过，不能把 Linux 探针当作全平台验收。
