...
架构确认：2026-09-16；状态更新：2026-09-17。状态：用户已确认架构基线，前端采用 Vue 3；应用未实现，V01–V04 已有 Linux 可控集成探针证据，见 [验证记录](validation.md)。本文细化 [MVP 规格](mvp-spec.md) 第 9、11 节，不替代其产品行为契约。已确认的 C01–C14 保持不变；本次确认覆盖五层职责、源码依赖、包映射和本文的架构默认方案；当前采用探针支持的 pi RPC + 每 run 一个独立进程，pi/MWF 版本已固定；实施基线及未完成项见 [实施交接](implementation-handoff.md)。架构确认不表示技术验收已通过，也不扩大尚未确认的产品交互或视觉范围。

用户已确认的补充约束：

- 优先支持 **Linux 和 macOS**，以 Linux 为当前开发与验证基线；按用户 2026-09-17 决定，macOS 实机验证延后至 MVP 完成后在用户本地电脑执行，不阻塞当前探针阶段收尾与 MVP 开发。Windows 暂不纳入。
...
| 内核隔离 | 独立子进程，采用已验证的 pi RPC；有已确认能力缺口时再评估 SDK worker | 内核异常与全局队列分离；需要管理进程生命周期与流协议 |
...
Vue 3、TypeScript、Node.js、SQLite 和 HTTP+SSE 已确认为技术组合；探针已固定 pi/MWF 及验证依赖；正式应用使用锁定的 npm workspaces 依赖、Node.js 原生 HTTP 与 node:sqlite；地图采用 SVG + HTML 自动布局，不引入地图组件库。首版优先支持 Linux 和 macOS；子进程收束须在两个平台分别验证，不能用 Linux 结果代替 macOS 结果。
...
vendor/pi/                      已固定接入的原版子模块
probes/                         V01–V04 真实集成验证
```

应用 workspace 包仍属计划，尚未创建；vendor/pi 与 probes 已存在。各包通过自己的公开入口导出，禁止跨包引用 `src/internal` 等私有文件。Domain 不通过一个大 `shared` 包间接获得平台依赖；contracts 只保留网络可传输的数据与验证 schema，不导出数据库行、内核事件或完整领域对象。Transport 显式完成应用结果与公共协议的映射。
...
分层的实际验收标准：拿掉 Web 可以通过用例测试驱动应用；拿掉 pi 可用替身验证队列规则；更新 pi 协议不需要在 UI、领域层搜索并改写原生事件类型；Linux/macOS 差异集中在平台实现。M1–M3 的架构检查、领域测试及真实适配测试已验证这些边界；完整应用验收仍由 M5 汇总。
...
- **当前采用 RPC**：Linux 探针已验证会话恢复、分叉边界、图片、提问和正常取消；硬终止后的进程收束仍需平台监督实现。协议解析、请求关联、stdout/stderr 分离均放在 Pi Adapter。
...
推荐一个活动 run 对应一个受监督执行进程，结束后关闭；继续会话从原生历史恢复。它比长期缓存每个 session 进程更容易核验隔离，但有启动开销。V01 已记录此机器约 1 秒的单次启动/恢复样本（含扩展加载），不作为性能基准；当前维持每 run 一个进程，不引入常驻复用。等待回答期间进程继续存在并占用名额。读取/分叉等辅助进程同样受监督，写操作仍持有分支操作权。
...
建议保留用户原有 pi 配置语义，通过 Configuration Module 调用已验证的配置入口；V03 已验证多个 worktree 的项目设置独立覆盖及项目信任边界，应用应保持这些语义。应用与外部 pi 同时编辑时需比较修订，写入只改目标字段。凭据不进入事件、URL、草稿、日志和 MWF。

MWF 沿用每个 worktree 的原生目录与跟踪选择。应用只协调 UI 纠正与关键交接保存；agent 按需召回/写入走固定版本的 MWF runtime 与扩展。二者必须尊重同一记忆写入锁和修订规则。每个 run 的日志与交接由应用单独保存，不覆盖 MWF 的单份项目 handoff，也不发明 MWF schema 字段。V04 已验证 add 的幂等回执及锁内修订适配；修订入口没有原生 request_id 回执，仍须在应用保存任务中对账，不能声称重试天然安全。
...
建议维护一份构建清单，记录 pi commit、适配器版本、MWF/扩展版本、运行时版本、依赖锁与应用数据 schema 版本。子模块固定源码并不自动固定其全部依赖，构建必须使用对应锁文件。当前选定版本以 probes/versions.json 为准；研究文档中的其他 SHA 仅作历史参考。
...
用户确认前端改用 Vue 3，并确认当前架构。Q1–Q7 的架构决策已收敛，保留编号供追踪；Q4、Q5 已根据 Linux V01 证据收敛为每 run 一个进程与原生 RPC，不再作为待用户表态的架构问题。
...
| Q4 · 当前基线已收敛 | 每个 run 启动进程还是常驻复用？ | 每 run 一个独立进程，暂不复用 | V01 已有单次延迟样本，不是性能基准；正式进程监督仍待实现 |
| Q5 · Linux 探针支持 RPC | 原生 RPC 还是 SDK worker？ | 当前采用原生 RPC，不建立 SDK worker | 未发现已确认需求的 RPC 能力缺口；硬终止缺口在平台监督层，macOS 待验证 |
...
以下为架构确认时的历史记录，不代表当前进度：当时将前端改为 Vue 3，并记录用户对当前架构基线的确认，同步文档与可视化；没有添加子模块、安装依赖、建立应用骨架、调用模型或执行技术探针。下一阶段按第 9 节开展 V01–V04，将真实结果回写本文后锁定接入方式和具体版本。若探针发现与已确认约束冲突的真实缺口，应明确记录并调整方案，不能把架构确认当作兼容性通过。

## 12. 技术探针阶段更新

V01–V04 已按本节前述范围建立可复现 Linux 实验并分轮提交，应用实现尚未开始。实际固定版本、运行结果及缺口以 [验证记录](validation.md) 为准；第 11 节保留此前架构确认阶段的历史记录。当前实施入口见 [实施交接与遗留任务](implementation-handoff.md)。

当前继续采用原生 RPC；硬终止后 Bash 可能残留，平台监督不能只杀 pi 进程组。模型启动需核对真实可用目录。配置/MWF 修订检查已验证窄适配路径，但涉及固定版本内部 API，必须保留升级契约检查。DeepSeek 真实模型探针已通过，macOS 验证按用户决定延后至 MVP 完成后，不阻塞后续开发；不能把 Linux 探针当作全平台验收。


## 平台实施补充（2026-09-17）

用户接受 macOS 通过本机 Linux 环境（Docker Desktop/Lima）运行后端和工具，浏览器仍在本机使用。沿用 Linux 的进程归属与收束机制，不将 subreaper 或 `/proc` 假定为 macOS 原生能力。启动打包须明确项目/应用数据挂载、文件权限和仅本机网络入口；macOS 实机验证仍后置到用户电脑。此选择不改变进程收束证据和 A01–A16 的验收义务。

### M3 原生分叉持久化补充

执行继续使用 RPC。历史分叉通过 infra-pi 内受监督的固定版 SessionManager 窄包装器，调用与原生 before-user 分叉相同的树操作。原生无 assistant 的分叉会延迟落盘，包装器以原生 header/entries 生成完整快照并先发布不可变回执，收束后发布目标会话；启动对账复用回执，不重放分叉或模型。此辅助操作不加载扩展 hook，来源扩展数据和压缩节点随原生历史保留；用户消息及图片成为草稿。原生入口白名单及回归约束见 native-contracts.md。
