...
升级必须先停止接收新运行并收束活动进程，处理待保存及排队项，在静止状态备份应用数据、原生会话和相关配置。更改固定版本后重跑上述真实应用契约，再补齐受影响的配置、fork 和记忆场景；不能只通过编译或探针就替换正在使用的内核。手动升级清单见 [备份与升级](backup-upgrade.md)，A15 的应用级 0.84.1 → 0.85.1 固定样本及 MWF 契约证据见 [M5c](evidence/m5-acceptance.md)；不泛化为所有历史版本兼容。
...
Git `status` 可执行 fsmonitor/过滤器，因此普通仓库检查也必须在监督 worker 内完成。`repository-worker.ts` 执行身份查询、完整检查、绑定核验、worktree 对账及分支名称/起点解析；应用在启动前持久化 `inspect-repository`，待收束后才结束。无项目绑定的读取用 nullable laneId 表达，重启只核验旧进程并要求明确重试，不伪造已添加项目。该新增操作类型属于预发布数据的增量扩展，旧代码不保证能理解它；降级仍遵守备份清单而非直接覆盖应用。

回归见 `tests/git.test.ts`、`tests/harness.test.ts` 与 `tests/recovery.test.ts` 的元数据/fsmonitor 场景。原生 Git 配置继续生效，不通过禁用钩子来隐藏子进程。
