...
已确认方案见 [技术架构 v0.3](architecture.md)，包括五层职责、依赖边界、包映射与跟随 pi 更新策略。前端采用 Vue 3 + TypeScript，后端采用 Node.js，应用元数据采用 SQLite，浏览器通信采用 HTTP+SSE；优先 Linux/macOS、手动检查更新，omp 暂缓。架构确认不改变本规格的 C 约束；pi/MWF 版本已固定，当前采用 RPC + 每 run 一个独立进程；应用依赖版本与地图组件在实施时选定，V01–V04 当前已有 Linux 可控集成探针，DeepSeek 真实模型已验证，macOS 实机验证延后至 MVP 完成后在用户本地电脑进行，不阻塞当前开发，详见 [验证记录](validation.md)。
...
- 当前采用 Linux 探针支持的原生 RPC；只有发现已确认需求的明确能力缺口时，才评估独立 SDK worker。浏览器不直接连接 pi stdio 或写配置文件。
...
以下保留阶段顺序；V01–V04 已有 Linux 探针证据，下一步为第 5 项应用纵向闭环。当前基线、剩余任务及完成条件见 [实施交接](implementation-handoff.md)，不将探针通过等同于应用验收：
...
历史说明（2026-09-16，仅设计提交阶段）：当时仅提交设计文档与静态线框，曾撤回临时 pi 子模块，尚未构建内核或执行探针。此后已固定子模块并完成 Linux 探针及 DeepSeek 真实模型验证；应用仍未开发，完整 A01–A16 应用验收尚未执行。当前阶段以 [实施交接](implementation-handoff.md) 为准，历史过程见 [设计评审记录](design-review.md)。
