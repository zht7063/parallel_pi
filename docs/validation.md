...
2026-09-17 更新。本文件记录实际实验，设计文档中的验收范围仍然有效。
...
- 真实 pi 进程加可控 provider；另已完成 DeepSeek 真实模型验证，见 V01 补充。macOS 没有运行证据。
...
2026-09-17 补充：使用用户授权的临时凭据，官方 `/models` 返回 `deepseek-flash` 与 `deepseek-v4-pro`；固定 pi 目录已包含前者及图片能力，无需更换快照或修改上游。执行 `node --test probes/v01-live.test.mjs`，DeepSeek `deepseek-flash` 的文字标记、真实 Bash 工具、红色图片理解、关闭并重开原生会话后的标记回忆全部通过（1 个集成测试，0 失败、0 跳过；总计约 9.26 秒）。见 [脱敏输出](../probes/results/deepseek-live-linux.txt) 和 [环境与源码哈希](../probes/results/deepseek-live-linux.json)。凭据仅传入进程环境，临时会话已清理。单次结果不代表稳定性或性能基准；macOS、真实 provider 的跨版本历史会话仍未验证。
...
限制：配置包装器使用固定 pi 的内部路径，应保留升级契约测试；非合作外部编辑器不遵守原生文件锁，尚不能声称任意编辑器的检查/写入竞态都已解决。本节配置冲突实验未使用真实凭据；真实调用见 V01 补充，macOS 尚待验证。
...
1. **真实 provider 范围**：DeepSeek `deepseek-flash` 单次文字、工具、图片和恢复已通过；不代表所有 provider 或真实历史会话跨版本兼容。
2. **macOS（已排期延后）**：用户于 2026-09-17 明确决定，在 MVP 完成后于其本地电脑执行实机验证。此项不再阻塞当前 V01–V04 探针阶段收尾或 MVP 开发；届时运行同一套安装与验证命令，重点检查取消、崩溃、残留工具、文件锁与恢复。验证前仍标记未验证，不能用 Linux 结果替代。
...
实施跟踪：上述剩余门槛已映射到 [实施交接 I01–I07](implementation-handoff.md)，包含所属模块、完成条件与 A01–A16 对应关系。该映射于 2026-09-17 文档收尾时建立，不表示新增运行验证或完成应用实现。
