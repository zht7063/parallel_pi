...
命令：`node --test probes/v02.test.mjs`。Linux 10 项实验通过，记录见 `probes/results/v02-linux.tap`。
...
范围限制：协调器是实验实现，没有正式应用分层、完整公平性/等待回答调度、通用进程身份核验或自动对账；没有 macOS 证据。不能据此宣称全部 A01–A16 已通过。
