...
尚未关闭（M5i 后）：真实外部模型应用文字/工具/图片/重启继续；最终保留运行归档与最终交付报告。macOS 实机验证按用户确认在 MVP 后进行，不将 Linux 或 Lima 配置校验冒充 macOS 实测。

### M5i：关闭最后网页后的后台执行与重开

扩展现有真实后端/原生 pi 提问浏览器测试，发送第二个工具任务并等待原生提问，保存未发送草稿后关闭最后一个页面，直接断言浏览器 context 已无页面、后端仍为 waiting_input。通过应用接口回答该 run，让真实 Bash 工具在没有网页连接时完成。随后重新创建页面，原会话恢复，草稿和原用户消息保留，显示执行完成；全部 run ID 恰好为原来的两个，没有重发。此场景使用受控 provider，明确验证网页与后台生命周期独立，不代表关闭整个后端或重启操作系统。

专项浏览器 1 项通过（18.7 秒，`/tmp/parallel-pi-m5i-browser.log`），类型与格式检查通过。只扩展测试，没有修改产品代码。A08 的独立关闭/重开证据补齐；外部模型与最终交付仍单独跟踪。
