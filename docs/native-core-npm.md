...
### N1d：Mac 平台专项通过，收束剩余四项测试

用户第二份日志对应 603723f：Mac 平台专项通过；完整套件 67 通过、1 失败、3 取消、2 Linux 专属跳过。上一轮路径别名、备份和后端 SIGKILL 回收断言已通过。仍未进入 npm 打包，不能声明安装验收通过。

剩余 Git 失败为测试竞态：started 事件不保证钩子已经写入 hook-count。用钩子写入前延迟 0.2 秒，在 Linux 确定性复现同一 ENOENT；测试现在有界等待该真实标记后再中断，保留 exactly-once 与恢复断言，延迟留作回归覆盖。

三个取消项是多步骤组合测试的 20/30 秒总时限。拆分为具名子阶段，分别保留 20/30 秒阶段上限，父测试预算调整为容纳多个串行阶段；产品 RPC、查询和监督超时完全未改。不能仅凭调整预算认定 Mac 卡顿已修复，下一轮必须查看各阶段实际结果。新增 `verify-macos.sh --focus`：运行平台专项及这四项重点检查，成功也不打包、不宣称完整验收。

Linux 验证：四项重点测试及其子阶段合计 16 项通过，0 失败/跳过/取消（约 26 秒）；类型、格式、shell 语法及差异检查通过。本轮只修改测试与验收脚本，没有修改产品代码；没有无理由重跑已通过的全应用或 npm 回归。日志 `/tmp/pi-mac-stages-tests.log`。
