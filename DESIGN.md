...
状态：MVP v0.1 的三张静态 SVG 线框已获用户接受；M1–M3 应用骨架、真实执行与共享关系地图已接入；M4 配置/成果界面已接入，M5 全量验收进行中。
...
Git 提交沿用 `GitChangesDialog.vue`：整文件复选框、提交消息、实际提交内容预览与明确确认依次展示；部分暂存文件不可新增选择，失效或消失的旧选择可以移除。失败保留输入，真实钩子进度和结果来自持久任务，刷新后重开仍可查看。不确定结果提供再次核对和明确外部核对两个操作；外部核对必须显示保留当前 Git 状态的确认，结束后仍需用户恢复队列。共享 AppDialog 负责未保存退出保护，使用现有 token 和就地反馈。证据为 `tests/git-commit.spec.ts`，390px 下确认按钮可达且无横向溢出。
