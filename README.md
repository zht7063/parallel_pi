...
当前已完成 MVP 设计基线，技术架构已确认，前端采用 Vue 3 + TypeScript，已完成 V01–V04 的 Linux 可控集成探针首轮，尚未开始应用开发。首版优先支持 Linux 和 macOS，手动检查上游更新；omp 支持暂缓。原版 pi 已通过 Git 子模块固定接入；RPC、工作区调度、配置和 MWF 路径已有运行证据；真实模型与 macOS 验证尚待完成，不能视为全量验收通过。
